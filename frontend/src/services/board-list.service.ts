...
export class BoardListService {
    ...
    API_URL: string = 'http://localhost:8080/api/board-lists';
    private http: HttpClient = inject(HttpClient);

    public getById(id: string): Observable<BoardListModel> {
        return this.http.get<BoardListModel>(`${this.API_URL}/${id}`);
    }

    public create(data: BoardListModel): Observable<BoardListModel> {
        return this.http.post<BoardListModel>(`${this.API_URL}`, data);
    }

    public update(id: string, data: BoardListModel): Observable<BoardListModel> {
        return this.http.put<BoardListModel>(`${this.API_URL}/${id}`, data);
    }
